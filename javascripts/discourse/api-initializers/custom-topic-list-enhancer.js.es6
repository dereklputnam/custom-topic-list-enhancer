...
export default apiInitializer("0.11.1", (api) => {
  ...
  ajax("/site.json").then((siteData) => {
    ...
    // Function to validate category IDs for a solution (silent validation)
    function validateSolutionCategories(solutionConfig) {
      ...
      return {
        ...
        hasInvalidIds: invalidLevel4.length > 0 || invalidLevel3.length > 0
      };
    }
    ...
    function updateSubscribeButton() {
      ...
      if (level4Ids.length === 0 && level3Ids.length === 0) {
        ...
        btn.title = "Check admin settings for category configuration";
      }
